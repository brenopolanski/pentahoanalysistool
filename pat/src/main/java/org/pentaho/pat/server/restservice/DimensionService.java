...
public class DimensionService {
	...
	@Produces({ "application/x-javascript", "application/xml",
			"application/json" })
	public JSONWithPadding moveDimension(
			@QueryParam("sessionId") String sessionId,
			@QueryParam("queryID") String queryID,
			@QueryParam("axis") String axis,
			@QueryParam("dimensionName") String dimensionName,
		...
		try {
			qs.moveDimension(sessionId, queryID, IAxis.Standard.valueOf(axis),
					dimensionName);
		...
		String bool = "true";
		return new JSONWithPadding(new GenericEntity<String>(bool) {
		}, jsoncallback);
	}
