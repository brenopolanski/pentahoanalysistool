...
public abstract class AbstractServiceIntegrationTest extends TestCase {
    ...
    protected void initTestContext() {
        if (!IS_INIT_DONE) {
            try {
                ...
                testProps.loadFromXML(AbstractServiceIntegrationTest.class
                ...
                slurp(stm, AbstractServiceIntegrationTest.class
    ...
    protected void initDatabase()
    {
        try 
        {
            ...
            Statement stm = c.createStatement();
            slurp(stm, AbstractServiceIntegrationTest.class.getResourceAsStream("pat-delete.sql")); //$NON-NLS-1$
            stm.executeBatch();
            stm.clearBatch();
            stm.close();
            c.commit();
            ...
            slurp(stm, AbstractServiceIntegrationTest.class.getResourceAsStream("pat-insert.sql")); //$NON-NLS-1$
            stm.executeBatch();
            stm.clearBatch();
            stm.close();
            c.commit();
