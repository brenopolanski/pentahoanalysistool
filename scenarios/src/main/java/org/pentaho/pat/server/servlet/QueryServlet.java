...
public class QueryServlet extends AbstractServlet implements IQuery {
    ...
    public CellDataSet alterCell(final String sessionId, final String queryId, final String connectionId, final String scenarioId,  
            int ordinal, final int newCellValue)throws RpcException {
        try {
            LOG.debug("Create Exclusion - SessionId: " + sessionId + " QueryId: " + queryId + " Scenario: " + scenarioId);
            return this.queryService.alterCell(getCurrentUserId(), sessionId, queryId, scenarioId, connectionId, ordinal, newCellValue);
        } catch (Exception e) {
