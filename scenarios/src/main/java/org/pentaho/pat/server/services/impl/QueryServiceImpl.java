...
public class QueryServiceImpl extends AbstractService implements QueryService {
    ...
    public CellDataSet alterCell(final String userId, final String sessionId, final String queryId, final String connectionId,
             final String scenarioId, int ordinal, final int newCellValue) {

        this.sessionService.validateSession(userId, sessionId);

        CellSet cellData = OlapUtil.getCellSet(queryId);

        final Cell cell = cellData.getCell(ordinal);

        cell.setValue(newCellValue, AllocationPolicy.EQUAL_ALLOCATION);

        CellDataSet cds = null;
        try {
            cds= executeQuery(userId, sessionId, queryId);
